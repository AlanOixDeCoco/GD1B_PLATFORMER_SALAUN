...
import BossFloorManager from "../Level/BossFloorManager.js";
import FloorManager from "../Level/FloorManager.js";

export default class DungeonBossScene extends BehaviourScene {
    constructor() {
        super(SCENE_DUNGEON_BOSS, false);
    }
    ...
    create(){
        ...
        this.lights.enable().setAmbientColor(EFFECTS.bossAmbianceColor);

        // create the Floor manager
        var floorManagerContainer = this.add.container()
        this.MakeBehaviors(floorManagerContainer, {
            "floor_manager": new BossFloorManager(),
        });
        this._bossFloorManager = floorManagerContainer.GetBehaviour("floor_manager");
        this._bossFloorManager.SetGameManager(this._gameManager);

        // Tilemap
        this._tilemap = this.add.tilemap(MAP_DUNGEON_BOSS, 0, 0);
        ...
        uiObjectLayer.objects.forEach(uiElement => {
            if(uiElement.properties[0].value == "uiOrigin"){
                this.CreateUI(uiElement.x, uiElement.y, FLOOR_TYPES.boss, this._gameManager);
            }
        });
        ...
        spawnsObjectLayer.objects.forEach(spawn => {
            switch(spawn.properties[0].value){
                case "player":
                    ...
                    break;
                case "enemies":
                    this._bossFloorManager.SetSpawnArea(spawn);
                    break;
                default:
        ...
        interactObjectLayer.objects.forEach(interactObject => {
            switch(interactObject.properties[1].value){
                case "door": 
                    ...
                    this._bossFloorManager.SetExitDoor(doorController);
                    break;
                case "decorativeDoor":
    ...
    NextFloor(){
        this._bossFloorManager.destroy();
        this._playerManager._parent.GetBehaviour("player_UI_controller").HideUI();
        this.scene.pause();
